...
containers:
  - name: tomcat
    ...
    env:
      ...
      {{- if or .Values.catalinaOpts .Values.metrics.enabled }}
      - name: CATALINA_OPTS
  ...
  - name: jmx-exporter
    image: {{ template "tomcat.metrics.image" . }}
    ...
    command:
      ...
      - {{ .Values.metrics.ports.metrics | quote }}
      - /etc/jmx-tomcat/jmx-tomcat-prometheus.yml
    ports:
    {{- range $key, $val := .Values.metrics.ports }}
      - name: {{ $key }}
        containerPort: {{ $val }}
    {{- end }}
    {{- if .Values.metrics.resources }}
    resources: {{- toYaml .Values.metrics.resources | nindent 6 }}
    {{- end }}
    volumeMounts:
      - name: jmx-config
...
volumes:
  ...
  {{- end }}
  {{- end }}
  {{- if and .Values.metrics.enabled (or .Values.metrics.config .Values.metrics.existingConfigmap) }}
  - configMap:
      name: {{ include "tomcat.metrics.configmapName" . }}
    name: jmx-config
  {{- end }}
